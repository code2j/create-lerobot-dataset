...
class GradioLeRobotVideoRecorder(Node):
    def __init__(self):
        ...
        self.current_frame_wrist_left_for_ui = None
        ...
        self.res_wrist_left = "0x0" # 왼쪽 손목 카메라 해상도

        # 상태 및 액션 (6 joints + 1 gripper = 7) * 2 robots = 14
        self.latest_data = {
            "image": None,
        ...
        self.subscription_wrist_left = self.create_subscription(
            CompressedImage, '/left/camera/cam_wrist/color/image_rect_raw/compressed', self._wrist_left_callback, 10)

        # 오른쪽 로봇 조인트 상태 (7 joints)
        self.right_joint_subscription = self.create_subscription(
            JointState, '/right/joint_states', self._right_joint_state_callback, 10)

        # 왼쪽 로봇 조인트 상태 (7 joints) - 추가된 부분
        self.left_joint_subscription = self.create_subscription(
            JointState, '/left/joint_states', self._left_joint_state_callback, 10)
    ...
    def _right_joint_state_callback(self, msg):
        """오른쪽 조인트 데이터 수신 콜백"""
        current_joints = []
        ...
        if len(current_joints) == 7:
            with self.lock:
                ...
                self.latest_data["state"][:7] = joint_tensor
                self.latest_data["action"][:7] = joint_tensor

    def _left_joint_state_callback(self, msg):
        """왼쪽 조인트 데이터 수신 콜백"""
        current_joints = []
    ...
    def _wrist_left_callback(self, msg):
        """왼쪽 손목 카메라 데이터 수신 콜백"""
        np_arr = np.frombuffer(msg.data, np.uint8)
        img_raw = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)
        if img_raw is not None:
            self.res_wrist_left = f"{img_raw.shape[1]}x{img_raw.shape[0]}"
            with self.lock:
                rgb = cv2.cvtColor(img_raw, cv2.COLOR_BGR2RGB)
                self.latest_data["image_wrist_left"] = rgb
                self.current_frame_wrist_left_for_ui = rgb
    ...
    def _recording_loop(self):
        ...
        with self.lock:
            ...
            if self.latest_data["image_wrist_left"] is None:
                self.status_msg = "⚠️ 왼쪽 손목 카메라 데이터 없음"
                return # 왼쪽 손목 카메라 데이터 없으면 넘어감
            ...
            img_wrist_left_tensor = torch.from_numpy(self.latest_data["image_wrist_left"]).permute(2, 0, 1)

            # 프레임 추가
            self.dataset.add_frame({
                "observation.image": img_tensor,
                "observation.image_secondary": img_secondary_tensor,
                "observation.image_wrist_left": img_wrist_left_tensor,
    ...
    def update_ui_components(self):
        ...
        with self.lock:
            r_joints_deg = [np.rad2deg(val.item()) for val in self.latest_data["state"][:7]]
            l_joints_deg = [np.rad2deg(val.item()) for val in self.latest_data["state"][7:]]

            joint_str = (f"Right: J1:{r_joints_deg[0]:>5.1f}°|J2:{r_joints_deg[1]:>5.1f}°|J3:{r_joints_deg[2]:>5.1f}°|J4:{r_joints_deg[3]:>5.1f}°|J5:{r_joints_deg[4]:>5.1f}°|J6:{r_joints_deg[5]:>5.1f}°|G:{r_joints_deg[6]:.1f}°\n"
                         f"Left : J1:{l_joints_deg[0]:>5.1f}°|J2:{l_joints_deg[1]:>5.1f}°|J3:{l_joints_deg[2]:>5.1f}°|J4:{l_joints_deg[3]:>5.1f}°|J5:{l_joints_deg[4]:>5.1f}°|J6:{l_joints_deg[5]:>5.1f}°|G:{l_joints_deg[6]:.1f}°")

            # --- 라벨 텍스트 생성 ---
            main_label = f"Main (Kinect) | {self.res_main}"
            sub_label = f"Secondary Camera | {self.res_sub}"
            wrist_left_label = f"Wrist Left Camera | {self.res_wrist_left}"
        ...
        return (
            ...
            gr.update(value=self.current_frame_wrist_left_for_ui, label=wrist_left_label), # 라벨 업데이트
...
def launch_ui(server_name:str, port:int, dt:float):
    ...
    with gr.Blocks(title="LeRobot Collector v3.3") as demo:
        ...
        with gr.Row():
            with gr.Column(scale=2):
                with gr.Row():
                    ...
                    image_wrist_left_output = gr.Image(label="Wrist Left Camera")
        ...
        gr.Timer(dt).tick(
            ...
            outputs=[image_output, image_secondary_output, image_wrist_left_output, progress_bar, status_text, ep_count_display, joint_info_display]
        )
